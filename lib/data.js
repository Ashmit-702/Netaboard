...
export async function getConstituencies() {
  ...
  try {
    // is_archived may not exist yet (added by migration 011) — select it
    // defensively so this works before and after that migration runs.
    let constituencies = null;
    const withFlag = await sb.from("constituencies").select("id,name,state,is_archived").order("name");
    if (withFlag.error) {
      const plain = await sb.from("constituencies").select("id,name,state").order("name");
      constituencies = plain.data;
    } else {
      constituencies = withFlag.data;
    }
    ...
    return constituencies.map((c) => {
      ...
      return {
        ...
        // A constituency is treated as archive-only if explicitly flagged,
        // or if its only linked result comes from an archived election.
        is_archived_constituency: c.is_archived ?? (latest?.election?.is_archived ?? true),
  ...
  try {
    ...
    for (const row of data) {
      ...
      if (name && !seen.has(name)) {
        seen.add(name);
        rows.push({ name, slug: row.politician?.slug, role: row.politician?.role, price: row.price, change_pct: row.change_pct, reason: row.reason });
      }
    }
    return rows;
