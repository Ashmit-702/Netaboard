...
export async function getElectionWatch() {
  ...
  try {
    const { data: elections } = await sb
      .from("elections")
      .select("id,name,region,election_date,status,description,is_archived,is_demo,data_status")
      .order("election_date", { ascending: false });
    if (!elections?.length) return { election: null };

    const { data: allPreds } = await sb
      .from("predictions")
      .select("election_id,option_label,probability,recorded_at,model_name,confidence,methodology,source_snapshot_at")
