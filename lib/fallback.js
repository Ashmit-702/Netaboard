...
// getElection() has been retired — /predictions and /market now read
// through getElectionWatch() (see lib/electionWatch.js), which is the only
// place "which election is current" gets decided, and correctly excludes
// fabricated demo data. fallbackElection is no longer used.
...
// Real election these seat counts are from — Coalition Builder shows this
// explicitly rather than presenting them as a current tally.
export const fallbackPartiesElectionMeta = {
  name: "Bihar Legislative Assembly Election 2020", election_date: "2020-11-10", is_archived: true,
};
...
const rawFallbackClaims = {
  ...
  "rahul-gandhi": [
    {
      id: "r1", claim_type: "promise", text: "NYAY minimum income guarantee scheme", claimant: "Rahul Gandhi",
      evidence: [], verdicts: [{ id: "v10", status: "unverified", confidence: 20, reasoning: "Tracked but no structured evidence has been attached yet.", methodology: "No evidence attached yet. Formula: base confidence 20 when evidence count = 0.", verdict_source: "manual", created_at: "2026-08-01" }],
    },
  ],
};
...
const fallbackPoliticianMeta = {
  ...
  // rahul-gandhi was previously missing here while existing in the seeded
  // politicians + stock_prices tables — that mismatch was the cause of
  // Trending Netas linking to a 404. Every politician referenced anywhere
  // (seed, stock_prices, fallbackStocks) must have an entry in this map.
  "rahul-gandhi": { name: "Rahul Gandhi", role: "Leader of Opposition, Lok Sabha", party: { abbreviation: "Congress" }, bio: "Congress MP and party leader." },
};
...
// Both records are real historical results, flagged archive-only so they
// never present as the current constituency picture. See migration 011.
export const fallbackConstituencies = [
  { name: "Patna Sahib", state: "Bihar", current_rep: "Ravi Shankar Prasad", vote_share: 55.2, margin: 12.4, turnout: 58.6, election_name: "Lok Sabha General Election 2024", election_date: "2024-06-04", is_archived: true, is_archived_constituency: true },
  { name: "Raghopur", state: "Bihar", current_rep: "Tejashwi Yadav", vote_share: 51.8, margin: 6.2, turnout: 61.3, election_name: "Bihar Legislative Assembly Election 2020", election_date: "2020-11-10", is_archived: true, is_archived_constituency: true },
];

export const fallbackStocks = [
  { name: "Narendra Modi", slug: "narendra-modi", role: "Prime Minister of India", price: 187.4, change_pct: 3.1, reason: "142 mentions in the last 24h (wikipedia_pageviews:89, gdelt_articles:31, hackernews:4, mastodon:18)" },
  { name: "Nitish Kumar", slug: "nitish-kumar", role: "Chief Minister, Bihar", price: 92.1, change_pct: -1.4, reason: "58 mentions in the last 24h (wikipedia_pageviews:22, gdelt_articles:19, mastodon:17)" },
  { name: "Tejashwi Yadav", slug: "tejashwi-yadav", role: "Leader of Opposition, Bihar", price: 104.6, change_pct: 4.2, reason: "76 mentions in the last 24h (wikipedia_pageviews:31, gdelt_articles:28, hackernews:2, mastodon:15)" },
  { name: "Rahul Gandhi", slug: "rahul-gandhi", role: "Leader of Opposition, Lok Sabha", price: 78.3, change_pct: -0.6, reason: "49 mentions in the last 24h (wikipedia_pageviews:20, gdelt_articles:24, mastodon:5)" },
];
...
export function fallbackChanges() {
  // No fabricated election_prediction entry here — with no real Supabase
  // connection there's no real election data to report a change on, and
  // inventing one is exactly what this app is built not to do. The
  // remaining entries are real politicians' actual tracked record.
  const changes = [
    {
...
// Honest by design: with no real Supabase connection there is no real
// current election to report, and this app does not invent one to fill the
// space — same rule as production. selectElection() would correctly reject
// any fabricated demo election anyway (is_demo is always excluded), so
// there's no "illustrative" version of this that isn't just the same empty
// state production shows. The UI (ElectionWatchCard) already renders
// "No major election movement today. Explore the election archive →" for
// this case.
