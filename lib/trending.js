 ...
 */
export async function getCurrentStories({ query = "India politics", maxStories = 6, maxTopics = 8 } = {}) {
  let articles = [];
  try {
  ...
  if (!articles.length) return { stories: [], topics: [] };

  const scored = clusterArticles(articles)
    .map((c) => ({ cluster: c, ...scoreCluster(c) }))
    // A story needs either corroboration from 2+ outlets or genuine
    // freshness — this is what stops a single outlet's repeated coverage
    // from being ranked as nationally significant.
