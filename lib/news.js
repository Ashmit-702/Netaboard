...
// GDELT — zero signup, ever. Free global news-event database, good filler
// when you haven't set up any of the keyed sources yet.
async function fromGDELT(query) {
  try {
    const res = await fetch(
      `https://api.gdeltproject.org/api/v2/doc/doc?query=${encodeURIComponent(query)}&mode=artlist&maxrecords=8&timespan=1d&format=json`
    );
    if (!res.ok) return [];
    const data = await res.json();
    return (data.articles || []).map((a) => ({ title: a.title, url: a.url, source: a.domain || "GDELT" }));
  } catch { return []; }
}
