...
export function buildSummary(preds) {
  ...
  return {
    ...
    // Surfaced so the UI never has to guess or imply a model that doesn't
    // exist — model_name defaults to 'manual-estimate' at the DB level
    // (see migration 006), so this is always populated, never blank.
